...
function runAction(actionType: string, payload: any) {
  switch (actionType) {
    ...
    case "count_commits":
      return {
        count_commit: Array.isArray(payload?.commits)
...
async function deliverToSubscriber(
  ...
  for (let attempt = 1; attempt <= MAX_DELIVERY_ATTEMPTS; attempt++) {
    try {
      ...
      if (response.ok) {
        console.log(
          `Delivered Job ${jobId} to ${targetUrl} on attempt ${attempt}`,
        );
        return;
      }
      ...
      console.error(
        `Delivery failed for Job ${jobId} to ${targetUrl} on attempt ${attempt} with status ${response.status}`,
      );
    } catch (err) {
      ...
      console.error(
        `Delivery crashed for Job ${jobId} to ${targetUrl} on attempt ${attempt}:`,
        err,
      );
    }
  ...
  // all attempts failed
  console.error(
    `All delivery attempts failed for Job ${jobId} for ${targetUrl}`,
  );
}

//fetches all subscribers for a pipeline
//sends the processed result to each one
async function deliverJobResults(
  jobId: number,
  pipelineId: number,
  result: any,
) {
