...
metricsRouter.get("/", async (req, res) => {
  try {
    ...
    const [
      ...
      pool.query(
        `SELECT COUNT(*)::int AS count FROM jobs WHERE status = 'completed'`,
      ),
      pool.query(
      ...
      pool.query(
        `SELECT COUNT(*)::int AS count FROM job_deliveries WHERE status = 'success'`,
      ),
      pool.query(
        `SELECT COUNT(*)::int AS count FROM job_deliveries WHERE status = 'failed'`,
      ),
    ]);
