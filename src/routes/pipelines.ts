...
PipeLineRouter.get("/:pipelineId", async (req, res) => {
  ...
  //edge case
  if (!pipelineId)
    return res.status(400).send({ error: "pipelineId is required" }); // 400 for bad request
  ...
  if (result.rowCount === 0)
    return res.status(404).send({ error: "Pipeline not found" }); // 404 for not found
...
PipeLineRouter.put("/:pipelineId", async (req, res) => {
  ...
  if (action_type && !ALLOWED_ACTION_TYPES.includes(action_type))
    return res.status(400).send({ error: "action type is invalid" });
  ...
  try {
    ...
    if (err?.code === "23505")
      return res.status(409).send({ error: "source_key already exists" });
